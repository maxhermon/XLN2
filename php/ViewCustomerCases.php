...
function getCustomerCases($customerID, $searchBy = '', $searchTerm = '', $page = 1, $casesPerPage = 10, $sortBy = '', $sortOrder = 'ASC') {
    ...
    if (!empty($searchBy) && !empty($searchTerm)) {
        if ($searchBy == 'user_name') {
            ...
            $whereClause .= "d.deptName LIKE :searchTerm";
        } else {
...
<head>
    ...
    <title>View Customer Cases</title>
...
<main>
    <h2>Cases for <?php echo htmlspecialchars($customerInfo['name']); ?></h2>
    ...
    <table id="casesTable">
        <thead>
            <tr>
                <th><a href="?cid=<?php echo $customerID; ?>&searchBy=<?php echo urlencode($searchBy); ?>&searchTerm=<?php echo urlencode($searchTerm); ?>&sortBy=caseID&sortOrder=<?php echo ($sortBy == 'caseID' && $sortOrder == 'ASC') ? 'DESC' : 'ASC'; ?>">Case ID <?php if ($sortBy == 'caseID') echo $sortOrder == 'ASC' ? '<i class="fa-solid fa-arrow-up"></i>' : '<i class="fa-solid fa-arrow-down"></i>'; ?></a></th>
                <th><a href="?cid=<?php echo $customerID; ?>&searchBy=<?php echo urlencode($searchBy); ?>&searchTerm=<?php echo urlencode($searchTerm); ?>&sortBy=department_name&sortOrder=<?php echo ($sortBy == 'department_name' && $sortOrder == 'ASC') ? 'DESC' : 'ASC'; ?>">Department <?php if ($sortBy == 'department_name') echo $sortOrder == 'ASC' ? '<i class="fa-solid fa-arrow-up"></i>' : '<i class="fa-solid fa-arrow-down"></i>'; ?></a></th>
